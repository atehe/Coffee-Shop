...
AUTH0_DOMAIN = "dev-u1af1bql.us.auth0.com"
ALGORITHMS = ["RS256"]
API_AUDIENCE = "dev"
...
def get_token_auth_header():
    ...
    if auth_split[0].lower() != "bearer":
        raise AuthError(
            {
                "code": "invalid_header",
                "description": 'Authotization header must start with "Bearer"',
            },
            401,
        )
    elif len(auth_split) == 1:
        raise AuthError(
            {"code": "invalid_header", "description": "Token not in header"}, 401
        )
...
def check_permissions(permission, payload):
    ...
    if "permissions" not in payload:
        raise AuthError(
            {"code": "invalid_claims", "description": "Permission not included"}, 400
        )
